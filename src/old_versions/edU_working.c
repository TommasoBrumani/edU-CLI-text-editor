...
void doCopy (char***, row_t**, int*, int*, int, int, row_t**, int*);
void doDelete (char***, row_t**, int*, int, int, row_t**, int*);
void doPrint (char**, int, int, int);
void doUndo (int*, int*, int*, int);
void doRedo (int*, int*, int*, int);
void doBalance (char***, int*, row_t**, row_t**, int, row_t**, int*);

/* Command execution subfunctions */
void actualUndo (char***, int*, row_t**, row_t**, int, row_t**, int*);
void actualRedo (char***, int*, row_t**, row_t**, int, row_t**, int*);
...
int main ()
{
    ...
    do {
        ...
        if (command=='c') {
            ...
            doBalance (&docHead, &firstEmpty ,&undoHead, &redoHead, n, &leftovers, &leftoverSize);
            n=0;
            doCopy (&docHead, &undoHead, &size, &firstEmpty, ind1, ind2, &leftovers, &leftoverSize);
            undoSize++;
            ...
            doBalance (&docHead, &firstEmpty ,&undoHead, &redoHead, n, &leftovers, &leftoverSize);
            n=0;
            doDelete (&docHead, &undoHead, &firstEmpty, ind1, ind2, &leftovers, &leftoverSize);
            undoSize++;
            emptyRedo(&redoHead, &leftovers);
            leftoverSize+=redoSize;
            redoSize=0;
...
void doCopy (char***docHead, row_t**undoHead, int*size, int*firstEmpty, int ind1, int ind2, row_t**leftovers, int*leftoverSize)
{
    /* This function serves as a hub for the execution of change commands */

    int len, docS;
    char in[MAX], fullStop [FSSIZE];
    char *tmp, **docH;
...
void doDelete (char***docHead, row_t**undoHead, int*firstEmpty, int ind1, int ind2, row_t**leftovers, int*leftoverSize)
{
    ...
    if (ind2>=*firstEmpty) {
        ...
        for (i=ind1, top=ind2+1, firstE=*firstEmpty, docH=*docHead; top<firstE; i++, top++)
            docH[i]=docH[top];
        *firstEmpty=i;
    }
...
void doBalance (char***docHead, int*firstEmpty, row_t**undoHead, row_t**redoHead, int n, row_t**leftovers, int*leftoverSize)
{
    if (n>0) {

        actualUndo (docHead, firstEmpty ,undoHead, redoHead, n, leftovers, leftoverSize);

    } else if (n<0) {

        actualRedo (docHead, firstEmpty, undoHead, redoHead, n, leftovers, leftoverSize);
...
void actualUndo (char***docHead, int*firstEmpty, row_t**undoHead, row_t**redoHead, int n, row_t**leftovers, int*leftoverSize)
{
        ...
        if (type==COPY) {
            ...
            if (*firstEmpty<=ind1) {
                ...
                *firstEmpty=count+ind1;
                ...
                for (i=*firstEmpty-1, docH=*docHead; i>=ind1; i--) 
                    *(docH+i+diff)=*(docH+i);
...
void actualRedo (char***docHead, int*firstEmpty, row_t**undoHead, row_t**redoHead, int n, row_t**leftovers, int*leftoverSize)
{
    ...
    for (; n>0; n--) {
        ...
        if ((*redoHead)->type==COPY) {
            ...
            doDelete(docHead, undoHead, firstEmpty, (*redoHead)->ind1, (*redoHead)->ind2, leftovers, leftoverSize);
...
void popStack (char***docHead, row_t**stackHead, int*count, row_t**leftovers, int*leftoverSize)
{
    ...
    for (i=0, docH=*docHead; ind1<=ind2 && row[i]!=NULL; i++, ind1++) {
        docH[ind1]=row[i];
    }
